...
Dieses Dokument und Repository umfasst die Erstellung eines Data Warehouse mit Data Vault Schema mittels dbt und der Bibliothek automateDV. Am Ende dieses Tutorials haben Sie eine Datenpipeline entwickelt, die Daten aus verschiedenen Systemen in ein Data Vault Schema integriert und daraus einen Star-Schema Data Mart erzeugt. Es wird außerdem möglich sein, den Data Mart als Snapshot zu einem bestimmten Zeitpunkt zu erzeugen, d.h. den Datenbestand zu verwenden, der zu diesem Zeitpunkt aktuell war. Zur Kontrolle ihrer Arbeit können Sie ein automatisches Skript verwenden (siehe automated_runs.md). Die eingesetzte Basistechnologie ist dbt (data build tool) und die Bibliothek automateDV stellt lediglich dbt-Funktionen bereit, die im Kontext von Data Vault nützlich sind. Das Tutorial wird aufeinander aufbauen und ist dazu gedacht, dass selbst mitprogrammiert wird, um die Effekte in der eigenen Datenbank zu erleben. Sollte trotz Erklärung und Debuggings nicht das erwartete Verhalten auftreten, gibt es zu jedem Branch einen Git-Branch der ausgecheckt werden kann, um mit einem sauberem Entwicklungsstand weiterzumachen.
